...
export function sketch(p5){
    ...
    function moreSetup(){
        ...
        // Disabling biometric analytics container as it does not appear in mockups
    ...
    p5.updateWithProps = props => {
        if(filepaths.length == 0){ // This will be updated only once
            // axes = props.axes;
    ...
    let hueSlider, saturationSlider;
    ...
    function createGUI(){
        ...
        // Disabling axis choice. Preset color with hue = 0 and saturation = 100 (pure red)
        // whose brightness is affected by the biometrics, and whose 
        // other two attributes will be editable.
        ...
        // Slider for color hue
        tempContainer = p5.createDiv().parent(container).addClass('p5EpisodeGUI-column-item');
        p5.createP('Color Hue: 0').parent(tempContainer).addClass('p5EpisodeGUI-row-item');
        hueSlider = p5.createSlider(0, 360, 0, 1).parent(tempContainer).addClass('p5EpisodeGUI-row-item');
        hueSlider.changed(colorSliderCallback);
        p5.createP('360').parent(tempContainer).addClass('p5EpisodeGUI-row-item')

        // Slider for color saturation
        tempContainer = p5.createDiv().parent(container).addClass('p5EpisodeGUI-column-item');
        p5.createP('Color Saturation: 0').parent(tempContainer).addClass('p5EpisodeGUI-row-item');
        saturationSlider = p5.createSlider(0, 100, 100, 1).parent(tempContainer).addClass('p5EpisodeGUI-row-item');
        saturationSlider.changed(colorSliderCallback);
        p5.createP('100').parent(tempContainer).addClass('p5EpisodeGUI-row-item');

        // Initializing vis color
        setAxisColor();
    ...
    function colorSliderCallback(){
        setAxisColor();
        initializeVisuals();
    }

    function setAxisColor(){
        axisColor = p5.color(hueSlider.value(), saturationSlider.value(), 100);
    }
    ...
    function handleGradient(){
        ...
        // Disabling rectangles as we did not have them in mockup.
        // Might be subject to change.
        // p5.stroke('white')
        // p5.strokeWeight(1)
